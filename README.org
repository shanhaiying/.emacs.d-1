...
My own snippets for [[https://github.com/capitaomorte/yasnippet][Yasnippet]] is in ~/.emacs.d/my-snippets.
...
To toggle Chinese input method (ibus, for example), press `C-\` or run command `M-x toggle-input-method`.

* Personal custom.el (OPTIONAL)
